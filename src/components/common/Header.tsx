...
export default function Header() {
    ...
    return (
        <header className="w-full flex items-center justify-between px-6 py-4 bg-white shadow">
            <button className="text-2xl font-bold text-neutral-900" onClick={() => navigate("/")}>FEITTO</button>

            {!authData ? (
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => navigate('/login')}
                        className="px-4 py-2 text-primary-dark font-semibold hover:text-accent-yellow transition-colors"
                    >
                        Login
                    </button>
                    <button
